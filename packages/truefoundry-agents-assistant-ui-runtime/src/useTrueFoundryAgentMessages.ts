...
export type PreTurnFailurePhase =
...
/**
 * A send failure that happened before the outgoing user message was projected
 * into the thread and before the turns API was called.
 */
export class TrueFoundryPreTurnError extends Error {
  readonly phase: PreTurnFailurePhase;
  readonly cause: unknown;

  constructor(phase: PreTurnFailurePhase, cause: unknown) {
    super(
      cause instanceof Error ? cause.message : "Failed to prepare the turn.",
    );
    this.name = "TrueFoundryPreTurnError";
    this.phase = phase;
    this.cause = cause;
  }
}
...
export function requiredActionsFromActiveUpdate(
  ...
  const custom = update.metadata?.custom as
...
export function useTrueFoundryAgentMessages({
  ...
  const load = useCallback(async () => {
    ...
    if (
      lazilyCreatedSessionIdRef.current != null &&
      sessionId !== lazilyCreatedSessionIdRef.current
    ) {
  ...
  const sendTurn = useCallback(
    async (options: SendTurnOptions) => {
      ...
      const runPreTurnStep = async <T>(
        phase: PreTurnFailurePhase,
        step: () => Promise<T>,
      ): Promise<T> => {
        try {
          return await step();
      ...
      try {
        ...
        if (activeSessionId == null) {
          const { remoteId } = await runPreTurnStep(
            "session-initialization",
            async () => {
              if (initializeSessionRef.current == null) {
                throw new Error(
                  "Cannot send a turn without an active session.",
                );
              }
              return await initializeSessionRef.current();
            },
          );
        ...
        const conversationSessionId = await runPreTurnStep(
          "session-resolution",
          () =>
        ...
        const session = await runPreTurnStep("session-loading", () =>
          getSession(client, conversationSessionId, sessionOptions),
        );
        const turnHeaders = await runPreTurnStep(
          "turn-headers",
          async () => await getTurnHeadersRef.current?.(),
        );
        const streamHeaders =
