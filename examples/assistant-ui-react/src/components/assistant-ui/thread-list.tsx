...
> = ({ className, ...props }) => {
  return (
    <ThreadListPrimitive.Root
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-1 overflow-hidden p-3",
        className,
      )}
      {...props}
    />
...
export const ThreadListItems: FC<ComponentPropsWithoutRef<"div">> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pb-1",
        className,
      )}
      {...props}
    >
...
export const ThreadListLoadMore: FC = () => {
  return (
    <AuiIf condition={(s) => s.threads.hasMore || s.threads.isLoadingMore}>
      <AuiIf condition={(s) => s.threads.isLoadingMore}>
        <ThreadListLoadMoreSkeleton />
      </AuiIf>
      <AuiIf condition={(s) => s.threads.hasMore && !s.threads.isLoadingMore}>
        <ThreadListPrimitive.LoadMore asChild>
          <Button
            ...
            className="text-muted-foreground hover:bg-muted h-8 w-full justify-center rounded-md px-2.5 text-sm font-normal"
          >
            Load more
          </Button>
...
const ThreadListLoadMoreSkeleton: FC = () => {
  return (
    <div
      role="status"
      aria-label="Loading more threads"
      data-slot="aui_thread-list-load-more-skeleton"
  ...
  return groups.map((group) => (
    <div key={group.label} className="flex flex-col gap-1">
      <div
        ...
        className="text-muted-foreground px-2.5 pb-0.5 text-xs font-medium first:pt-0"
      >
        {group.label}
      </div>
...
export const ThreadListItem: FC = () => {
  return (
    <ThreadListItemPrimitive.Root
      ...
      className="group hover:bg-muted focus-visible:bg-muted data-active:bg-muted has-focus-visible:bg-muted has-data-[state=open]:bg-muted relative flex min-h-9 items-center rounded-lg transition-colors focus-visible:outline-none"
    >
      <ThreadListItemPrimitive.Trigger
        data-slot="aui_thread-list-item-trigger"
        className="focus-visible:ring-ring/50 flex min-h-9 min-w-0 flex-1 items-center rounded-lg px-2.5 py-2 text-start text-sm outline-none group-hover:pe-9 group-has-focus-visible:pe-9 group-has-data-[state=open]:pe-9 group-data-active:pe-9 focus-visible:ring-[3px]"
      >
        <span
