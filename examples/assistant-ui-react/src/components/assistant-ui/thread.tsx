...
import { AssistantLeafPart } from "@/components/assistant-ui/assistant-leaf-part";
...
import { threadHasPendingMcpAuth } from "@/lib/chat/mcpAuthView";
import {
  ...
  return (
    <ThreadPrimitive.ScrollToBottom asChild>
      <TooltipIconButton
        ...
        variant="outline"
        className="aui-thread-scroll-to-bottom dark:border-border dark:bg-background dark:hover:bg-accent absolute -top-14 z-10 self-center rounded-full p-4 disabled:invisible"
      >
        <ArrowDownIcon />
      </TooltipIconButton>
...
const ComposerArea: FC = () => {
  ...
  if (mcpPending) {
    return <McpAuthContinueComposer />;
  }
  if (toolResponsesPending.length > 0) {
    return <AskUserComposer />;
  }
  return <Composer />;
};
...
const McpAuthContinueComposer: FC = () => {
  const { resume } = useTrueFoundryMcpAuth();
  const disabled = useAuiState((s) => s.thread.isRunning);

  return (
    <div
      data-slot="aui_mcp-auth-continue-composer"
      className="border-border/60 flex w-full flex-col gap-3 rounded-(--composer-radius) border bg-(--composer-bg) p-(--composer-padding) shadow-[0_4px_16px_-8px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-none"
    >
...
const AskUserComposer: FC = () => {
  ...
  const submitTypedAnswer = () => {
    const content = typedAnswer.trim();
    if (!content) {
      return;
    }
  ...
  return (
    <div
      data-slot="aui_ask-user-composer"
      className="border-border/60 flex w-full flex-col gap-3 rounded-(--composer-radius) border bg-(--composer-bg) p-(--composer-padding) shadow-[0_4px_16px_-8px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-none"
    >
      <p className="text-foreground px-2.5 text-sm font-medium">
      ...
      {options.length > 0 ? (
        <div className="flex flex-wrap gap-2 px-2.5">
          {options.map((option) => (
            <Button
              ...
              disabled={disabled}
              onClick={() =>
                respond({ toolCallId: item.toolCallId, content: option })
              }
            >
              {option}
            </Button>
        ...
        <div className="flex flex-col gap-2 px-2.5">
          <input
            ...
            disabled={disabled}
            className="border-border/60 rounded-md border bg-transparent px-3 py-2 text-sm outline-none"
          ...
          <div className="flex justify-end">
            <Button
              type="button"
              className="rounded-full px-4"
              disabled={disabled || typedAnswer.trim().length === 0}
