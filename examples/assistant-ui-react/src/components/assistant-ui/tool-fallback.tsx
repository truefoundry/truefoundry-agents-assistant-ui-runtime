...
function hasPendingToolApproval(
  approval: ToolCallMessagePart["approval"] | undefined,
): boolean {
  return (
    approval != null &&
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  const showApproval = isRequiresAction && hasPendingToolApproval(approval);

  const nestedToolCallMessageComponents = useMemo(
    () => ({
      AssistantMessage: function NestedSubAgentAssistantMessage() {
        ...
        return (
          <>
            ...
            <MessagePrimitive.Root data-role="assistant" className="px-1 py-0.5">
              <MessagePrimitive.Parts
                components={{
                  ...assistantMessagePartComponents,
                  tools: {
                    Fallback: (toolProps: ToolCallMessagePartProps) => (
                      <ToolFallback
                        {...toolProps}
                        respondToApproval={(response) => {
                          if (toolProps.approval == null) {
                            return;
                          }
                          const approved =
                          ...
                          respondToNestedApproval({
                            approvalId: toolProps.approval.id,
                            approved,
                          });
                        }}
                      />
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen}>
      ...
      <ToolFallbackContent>
        ...
        <MessagePartPrimitive.Messages components={nestedToolCallMessageComponents} />
      </ToolFallbackContent>
    </ToolFallbackRoot>
  );
};
