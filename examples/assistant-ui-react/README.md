...
3. **Configure environment variables** in `examples/assistant-ui-react/.env.local`:

   ```
   NEXT_PUBLIC_TF_CP_URL=https://<your-control-plane-host>
   NEXT_PUBLIC_TF_GATEWAY_URL=https://gateway.truefoundry.ai/<your-tenant>
   NEXT_PUBLIC_TF_AGENT_NAME=your-agent-name
   ```

   Auth is handled separately via device or cookie flow (`AuthProvider`). The gateway URL and default agent name are read from env at build time.
...
| `src/lib/chat/gatewayCredentials.tsx` | `useGatewayCredentials()` — merges auth token with env gateway config |
