...
| Tool approvals | Yes | `[y] Allow / [n] Deny` prompt (`useApprovals`). |
| Ask-user prompts | Yes | Free-text answers and `↑`/`↓` option lists (`useToolResponses`). |
| MCP OAuth | Yes | Prints authorize links, resumes on Enter (`useMcpAuth`). |
...
| File attachments | No | Runtime supports it; `agentAttachmentAdapter` not wired. |
| Nested sub-agent messages | No | Runtime supports it; tool row shows status only, no nested `part.messages`. |
| Turn cancellation | No | Runtime supports it; `useCancel` not wired — `Ctrl+C` just exits. |
